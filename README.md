...
| 平台 | 安装包 | 支持范围 |
| --- | --- | --- |
| macOS | `Workday-Island-v0.9.0-macOS-universal.dmg` | macOS 12+，Apple Silicon（M 系列）与 Intel |
| Windows | `Workday-Island-v0.9.0-windows-x64-Setup.exe` | Windows 10/11 x64，依赖 Microsoft Edge WebView2 |
...
- **实时聊天、好友与窗口互动**：支持一键匿名设备身份、账号注册或用户名密码登录；注册时邀请码可选，可按精确用户名/用户 ID 添加好友、处理好友申请、查看在线状态，并发送消息、抖动或闪烁窗口。
- **本地优先**：设置、待办、薪资和专注状态只保存在本机；账号密码仅用于当前 WebSocket 在线会话，不写入本地数据文件。
...
可选择匿名设备、注册新账号或使用账号密码登录，通过好友列表快速聊天，并支持申请处理、在线状态、抖动、闪烁与提示语等窗口互动。
...
9. 点击顶部聊天按钮，可选择匿名“一键登录”、注册新账号或用户名密码登录。注册时邀请码为选填；上线后可按精确用户名/用户 ID 发送好友申请，从好友列表直接进入会话；仍可手动输入用户 ID 聊天、抖动或闪烁对方窗口。详情见[实时聊天说明](docs/REALTIME_CHAT.zh-CN.md)。
...
- Ed25519 设备签名、用户名密码挑战认证、好友协议与 WebSocket 实时通信
