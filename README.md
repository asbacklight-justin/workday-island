...
| 平台 | 安装包 | 支持范围 |
| --- | --- | --- |
| macOS | `Workday-Island-v0.5.0-macOS-universal.dmg` | macOS 12+，Apple Silicon（M 系列）与 Intel |
| Windows | `Workday-Island-v0.5.0-windows-x64-Setup.exe` | Windows 10/11 x64，依赖 Microsoft Edge WebView2 |
...
- **精简模式**：采用 2×2 核心卡片布局；窗口可在 400×270 至 900×600 之间调整，内部字号、间距与组件同比缩放。
- **下班倒计时**：自定义上下班时间与每周工作日，实时展示剩余时间和当日进度。
- **今日已赚**：根据月薪、月计薪天数与当天工作进度估算；不填写月薪时自动隐藏，不收集薪资数据。
...
- **天气**：通过 Open-Meteo 查询指定城市的当前天气，无需 API Key。
...
6. 点击顶部“精简”切换 2×2 小窗；拖动窗口边缘即可缩放。
...
待办、提醒、薪资、工作时间和专注记录只写入本机 JSON 文件。天气功能会访问 Open-Meteo，并发送用户填写的城市名称及解析后的坐标；除此之外应用没有账号系统、遥测、广告 SDK 或自有服务端。详情见 [隐私说明](docs/PRIVACY.zh-CN.md)。
