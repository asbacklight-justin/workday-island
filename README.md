...
一款常驻桌面的轻量工作助手：把时间、下班倒计时、天气、待办、持续提醒、专注计时、英语学习、好友聊天和工作云盘集中到一座安静的“工位岛”上。
...
| 平台 | 安装包 | 支持范围 |
| --- | --- | --- |
| macOS | `Workday-Island-v0.10.0-macOS-universal.dmg` | macOS 12+，Apple Silicon（M 系列）与 Intel |
| Windows | `Workday-Island-v0.10.0-windows-x64-Setup.exe` | Windows 10/11 x64，依赖 Microsoft Edge WebView2 |
...
- **今日英语学习**：点击顶部 `EN` 按钮打开紧凑学习窗，支持单词学习、双语例句、英译中四选一、中译英四选一和完整单词拼写，并即时显示答案对错与上一词；透明度仅影响背景，学习内容始终清晰。
...
- **统一账号入口**：登录和注册从聊天页移至独立账号中心；登录一次即可共享聊天、好友和工作云盘，退出账号时相关在线服务同步退出，工作台会展示当前昵称。
...
9. 点击顶部账号按钮登录或注册。邀请码为选填；登录后聊天和工作云盘共享会话，退出账号时两者同时退出。
...
12. 点击顶部 `EN` 按钮进入今日英语学习；可切换五种学习模式，并在设置中选择所用词库。
...
- Backlight 账号认证、好友协议、WebSocket 实时通信与对象存储直传

桌面客户端可在 `workday-island` 目录独立构建与发布；待办、倒计时等本地功能不依赖服务器，账号、聊天、英语在线词库与工作云盘功能使用 Backlight 服务。
...
待办、提醒、薪资、工作时间和专注记录只写入本机 JSON 文件。天气访问 Open-Meteo，行情访问东方财富公开接口，版本检查访问 GitHub；用户主动使用账号、聊天、英语学习或工作云盘时，相应数据会发送到 Backlight 服务。应用没有遥测或广告 SDK。详情见 [隐私说明](docs/PRIVACY.zh-CN.md)。
