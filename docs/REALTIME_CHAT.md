...
5. Signing out from the account centre closes realtime chat and signs out of Work Cloud and Universal Translator. The password and access token are cleared from process memory, so online services require a new sign-in next time.
...
The desktop client opens an encrypted WebSocket at `wss://admin.asbacklight.cn/api/realtime/ws`. Account sign-in uses `https://admin.asbacklight.cn/api/user/login`, while registration uses `https://admin.asbacklight.cn/api/user/register`. These handshakes and business requests include `X-Client-Source: workday-island` and the current `X-Client-Version`.
